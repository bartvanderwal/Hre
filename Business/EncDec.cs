...
// BS 2011-06-28: This file is copied from http://www.codeproject.com/KB/security/DotNetCrypto.aspx
// BW 2011-07-12: Added optional URL encoding/decoding and use of salt in decryption and encryption and made 'unused' functions private.
...
namespace HRE.Data {
    ...
    public class EncDec {
        ...
        private static byte[] Encrypt(byte[] clearData, string Password) {
            ...
            // (i.e. trying to guess a password by enumerating all possible words).
            PasswordDeriveBytes pdb = new PasswordDeriveBytes(Password,
                new byte[] {0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76});
            ...
            // Using PasswordDeriveBytes object we are first getting 32 bytes for the Key 
            // (the default Rijndael key length is 256bit = 32bytes) and then 16 bytes for the IV. 
            // IV should always be the block size, which is by defaul 16 bytes (128 bit) for Rijndael. 
            // If you are using DES/TripleDES/RC2 the block size is 8 bytes and so should be the IV size. 
            // You can also read KeySize/BlockSize properties off the algorithm to find out the sizes. 
        ...
        private static void Encrypt(string fileIn, string fileOut, string Password) { 
            ...
            // Now we will initialize a buffer and process the input file in chunks. 
        ...
        /// Decrypt function.
        /// </summary>
        public static string Decrypt(string cipherText, string Password, bool useUrlEncoding) {
            // Optionally URL decode the decrypted data (use if data was put in URL as URL parameter/querystring).
            if (useUrlEncoding) {
